...
<?php
    if (isset($_POST['telephone']))
    {
        ...
        if (preg_match("#^[0+][0]?([3]{2})?[1-68]([-. ]?[0-9]{2}){4}$#", $_POST['telephone']))
        {
...
<?php
    if (isset($_POST['email']))
    {
        ...
        {
            echo 'Le ' . $_POST['email'] . ' est une adresse email <strong>valide</strong> !';
        }
        else
        {
            echo 'L\'adresse email ' . $_POST['email'] . ' n\'est pas valide, recommencez !';
        }
    }
?>
</p>
...
<form method="post">
<p>
    <label for="email">Votre email ?</label> <input id="email" name="email" /><br />
